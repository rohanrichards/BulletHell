...
public partial class RaycastSystem : SystemBase
{
    ...
    public partial struct RaycastJob : IJobEntity
    {
        ...
        {
/*            UnityEngine.Debug.Log("distance: " + math.length(local.Position - target));
            UnityEngine.Debug.Log("LocalToWorld.Position: " + local.Position);
            UnityEngine.Debug.Log("Translation.Value: " + position.Value);
            UnityEngine.Debug.Log("target LocalToWorld.Position: " + target);*/
            if (math.length(position.Value - target) > 2)
            {
