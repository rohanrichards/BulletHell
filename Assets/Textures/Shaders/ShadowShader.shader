Shader "Unlit/ShadowShader"
{
    Properties
    {
        ...
        _SecondTex ("Second Texture", 2D) = "red" {}
        ...
        _MaxShadowLights ("Maximum Shadow Casting Lights", Int) = 25
    }
    SubShader
    {
...
//////////////////////////////
            ...
            bool IsShadowed(fixed4 col) {
                bool is_shadowed = (col.x > 0.0) || (col.y > 0.0) || (col.z > 0.0);
                return !is_shadowed;
            }
            ...
            fixed4 QuantizedShadowTex(float2 in_uv) {
                float2 uv = (floor(in_uv * _SecondTex_TexelSize.zw) + 0.5) * _SecondTex_TexelSize.xy;
                return tex2Dlod(_SecondTex, float4(uv, 0, 0));
            }
                ...
                for(int light = 0; light < _LightCount; light++) {
                    ...
                    if(light < _MaxShadowLights) {
                        for(int x = 0; x < NUM_STEPS; x++) {
                            ...
                            fixed4 shadow_lookup = QuantizedShadowTex(pos);
                            blocked += IsShadowed(shadow_lookup) ? 0.0 : pow(along_frac, _ShadowFade);
                            prev_pos = pos;
                        }
                    }

                    float normal_frac = 1.0 - ColGradient(col, QuantizedTex(prev_pos)) * _TextureBump;
